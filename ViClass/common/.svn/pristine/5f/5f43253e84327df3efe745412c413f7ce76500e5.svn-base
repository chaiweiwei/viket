...
@interface ALDBigPhotoView (Private)
- (UIImage*)createHighlightImageWithFrame:(CGRect)rect;
@end

@implementation ALDBigPhotoView
...
-(void) startActivityIndicator{
    UIActivityIndicatorView *actv=(UIActivityIndicatorView*)[self viewWithTag:kActTag];
    if (!actv) {
        [self addSubview:_activity];
    }
...
- (void)setImageWithURL:(NSString *)imageUrl placeholderImage:(UIImage *)placeholder {
    ...
    if ([imageUrl hasPrefix:@"http://"] || [imageUrl hasPrefix:@"https://"]) {
        ...
        UIImage *image=[imageLoad loadImageWithUrl:imageUrl delegate:self];
        if (image) {
            [self setImage:image];
        ...
        [self setImage:image];
    }else if(imageUrl && ![imageUrl isEqualToString:@""]){
